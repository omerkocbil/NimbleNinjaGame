...
class MLHero: SKSpriteNode {
    ...
    func loadAppearance() {
        ...
        let rightEye = SKSpriteNode(color: eyeColor, size: CGSize(width: 6, height: 6))
        let leftEye = rightEye.copy() as! SKSpriteNode
        ...
        leftEye.addChild(pupil.copy() as! SKSpriteNode)
    ...
    func fall() {
        ...
        let action = SKAction.rotate(byAngle: -CGFloat(M_PI)/2.0, duration: 0.4)
        run(action)
    }
